...
public class CourseOfferingController {
    ...
    public String showUpdateForm(@PathVariable("id") long id, Model model) {
        CourseOffering courseOffering = courseOfferingRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Invalid course offering Id:" + id));
        model.addAttribute("courseOffering", courseOffering);
        return "courseOfferings/update";
    }
    
    @PostMapping("/courseOfferings/update/{id}")
    public String update(@PathVariable("id") long id, @Valid CourseOffering courseOffering, BindingResult result, Model model) {
        if (result.hasErrors()) {
    ...
    public String delete(@PathVariable("id") long id, Model model) {
        CourseOffering courseOffering = courseOfferingRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Invalid courseoffering Id:" + id));
